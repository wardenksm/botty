...
class GameStats:
    ...
    def log_item_keep(self, item_name: str, send_message: bool, img: np.ndarray, ocr_text: str = None):
        ...
        filtered_names = ["GOLD", "DIAMOND", "SKULL"]
        filtered_items = [" POTION", "AMETHYST", "RUBY", "ED DIAMOND", "LESS DIAMOND", "TOPAZ", "EMERALD", "SAPPHIRE", "ED SKULL", "LESS SKULL"]
        if self._location is not None and item_name in filtered_names and not any(substring in item_name for substring in filtered_items):
