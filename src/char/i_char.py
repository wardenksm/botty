...
from ui_manager import is_visible, wait_until_visible, detect_screen_object, ScreenObjects
    ...
    def tp_town(self):
        ...
        while (time.time() - start) < 8:
            ...
            if (template_match := detect_screen_object(ScreenObjects.TownPortal, img)).valid:
                ...
                #gray = cv2.cvtColor(tp_img, cv2.COLOR_BGR2GRAY)
                #_, mask = cv2.threshold(gray, 254.5, 255, cv2.THRESH_BINARY)
                kernel = np.ones((2, 2), 'uint8')
                mask = cv2.erode(mask, kernel, None, iterations=1)
