...
class Hammerdin(Paladin):
    ...
    def kill_council(self) -> bool:
        ...
        self._cast_hammers(atk_len)
        has_tp = self.capabilities.can_teleport_natively or self.capabilities.can_teleport_with_charges
        if has_tp:
            ...
            for _ in range(4):
                ...
                # Move a bit back and another round
            #cv2.imwrite(f"./debug_screenshots/council_inside_{time.strftime('%m%d_%H%M%S')}.png", grab())
            self._cast_hammers(atk_len)
